...
var defaultModelRatio = map[string]float64{
	...
	"grok-4.5":              1, // $2 / 1M input tokens below the 200K prompt threshold
	"grok-4.5-latest":       1,
	"grok-build-latest":     1,
	// Kimi
	"kimi-k3": 0.02 * RMB, // ¥20 / 1M uncached input tokens
	// Z.AI
	"glm-5.2": 0.7, // $1.40 / 1M input tokens
	// submodel
...
func GetCompletionRatio(name string) float64 {
	...
	// An explicitly configured model ratio must take precedence over the
	// built-in family defaults. This also lets custom aliases such as
	// gpt-5.x-* use their configured output price.
