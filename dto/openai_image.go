...
func (i *ImageRequest) GetTokenCountMeta() *types.TokenCountMeta {
	...
	return &types.TokenCountMeta{
		...
		ImagePriceTier:  imagePriceTier,
	}
}

type gptImage2SizeTier uint8

const (
	gptImage2SizeTierInvalid gptImage2SizeTier = iota
	gptImage2SizeTier1K
	gptImage2SizeTier2K
	gptImage2SizeTier4K
)

// gptImage2SizePriceRatio maps the supported output dimensions onto the
// configured per-image price tiers. The model's base price is the 1K price;
// the returned multiplier produces 2K and 4K prices while the existing image
// billing path continues to apply the requested image count exactly once.
func gptImage2SizePriceRatio(size string) float64 {
	switch parseGPTImage2SizeTier(size) {
	case gptImage2SizeTier2K:
		return 2.5
	case gptImage2SizeTier4K:
		return 10.0 / 3.0
	default:
		return 1
	}
...
// IsGPTImage2SizeStrictlyAbove2K reports whether a size belongs to the 4K
// billing tier. Explicit 2K and custom dimensions with a longest edge of 2560
// are deliberately excluded.
func IsGPTImage2SizeStrictlyAbove2K(size string) bool {
	return parseGPTImage2SizeTier(size) == gptImage2SizeTier4K
}

func GPTImage2SizePriceTier(size string) (string, bool) {
	switch parseGPTImage2SizeTier(size) {
	case gptImage2SizeTier1K:
		return "1K", true
	case gptImage2SizeTier2K:
		return "2K", true
	case gptImage2SizeTier4K:
...
func parseGPTImage2SizeTier(size string) gptImage2SizeTier {
	normalized := strings.ToLower(strings.TrimSpace(size))
	switch normalized {
	case "", "auto", "1k":
		return gptImage2SizeTier1K
	case "2k":
		return gptImage2SizeTier2K
	case "4k":
		return gptImage2SizeTier4K
	}

	parts := strings.Split(normalized, "x")
	if len(parts) != 2 {
		return gptImage2SizeTierInvalid
	}
	...
	switch {
	case longestEdge <= 1536:
		return gptImage2SizeTier1K
	case longestEdge <= 2560:
		return gptImage2SizeTier2K
	default:
		return gptImage2SizeTier4K
	}
