...
type Channel struct {
	...
	WeightOverridden   bool   `json:"weight_overridden,omitempty" gorm:"-"`
...
const (
	ChannelLastTestErrorCodeKey    = "last_test_error_code"
	ChannelLastTestErrorMessageKey = "last_test_error_message"
	ChannelLastTestErrorTimeKey    = "last_test_error_time"
)

var channelOtherInfoLocks sync.Map

func getChannelOtherInfoLock(channelId int) *sync.Mutex {
	if lock, exists := channelOtherInfoLocks.Load(channelId); exists {
		return lock.(*sync.Mutex)
	}
	newLock := &sync.Mutex{}
	actual, _ := channelOtherInfoLocks.LoadOrStore(channelId, newLock)
...
func (options ChannelSortOptions) ApplyForGroup(query *gorm.DB, group string) *gorm.DB {
	group = NormalizeChannelGroupFilter(group)
	useGroupPriority := group != "" && (options.SortBy == "priority" || (options.SortBy == "" && !options.IDSort))
	if !useGroupPriority {
	...
	join := "LEFT JOIN channel_group_routings AS cgr_sort ON cgr_sort.channel_id = channels.id AND cgr_sort." + commonGroupCol + " = ?"
	return query.Joins(join, group).Order("COALESCE(cgr_sort.priority_override, channels.priority, 0) " + direction)
}
...
func channelGroupFilterCondition() string {
	if common.UsingMySQL {
		return `CONCAT(',', channels.` + commonGroupCol + `, ',') LIKE ? ESCAPE '!'`
	}
	return `(',' || channels.` + commonGroupCol + ` || ',') LIKE ? ESCAPE '!'`
}
...
func updateChannelMetadata(channelId int, selectedFields []string, mutator func(channel *Channel, otherInfo map[string]interface{}) error) (*Channel, error) {
	lock := getChannelOtherInfoLock(channelId)
	lock.Lock()
	defer lock.Unlock()
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := query.First(&updated, "id = ?", channelId).Error; err != nil {
			return err
		}

		otherInfo := updated.GetOtherInfo()
		if err := mutator(&updated, otherInfo); err != nil {
			return err
		}
		if len(otherInfo) == 0 {
			updated.OtherInfo = ""
			...
			updated.OtherInfo = string(otherInfoBytes)
		}

		return tx.Model(&updated).Select(selectedFields).Updates(&updated).Error
	})
	if err != nil {
		return nil, err
	}
...
// MergeChannelOtherInfo serializes read-modify-write updates to a channel's
// other_info document so independent metadata writers do not overwrite each
// other.
func MergeChannelOtherInfo(channelId int, mutator func(otherInfo map[string]interface{}) error) (*Channel, error) {
	return updateChannelMetadata(channelId, []string{"other_info"}, func(_ *Channel, otherInfo map[string]interface{}) error {
		return mutator(otherInfo)
	})
}

// MergeChannelOtherInfoAndName updates channel metadata and its display name in
// one transaction so probe snapshots and multiplier suffixes stay consistent.
func MergeChannelOtherInfoAndName(channelId int, mutator func(channel *Channel, otherInfo map[string]interface{}) error) (*Channel, error) {
	return updateChannelMetadata(channelId, []string{"name", "other_info"}, mutator)
}
...
func SearchChannels(keyword string, group string, model string, idSort bool, sortOptions ...ChannelSortOptions) ([]*Channel, error) {
	...
	// 执行查询
	err := order.ApplyForGroup(baseQuery, group).Find(&channels).Error
	if err != nil {
		return nil, err
	}
	return channels, nil
...
func BatchDeleteChannels(ids []int) error {
	...
	for _, chunk := range lo.Chunk(ids, 200) {
		if err := DeleteChannelGroupRoutings(tx, chunk); err != nil {
			tx.Rollback()
			return err
		}
...
func (channel *Channel) UpdateResponseTime(responseTime int64) {
	updated, err := updateChannelMetadata(channel.Id, []string{"response_time", "test_time", "other_info"}, func(current *Channel, info map[string]interface{}) error {
		delete(info, ChannelLastTestErrorCodeKey)
		delete(info, ChannelLastTestErrorMessageKey)
		delete(info, ChannelLastTestErrorTimeKey)
		current.TestTime = common.GetTimestamp()
		current.ResponseTime = int(responseTime)
		return nil
	})
	if err != nil {
	...
	channel.TestTime = updated.TestTime
	channel.ResponseTime = updated.ResponseTime
	channel.OtherInfo = updated.OtherInfo
}

func (channel *Channel) UpdateTestFailure(errorCode string, errorMessage string) {
	errorCode = strings.TrimSpace(errorCode)
	if errorCode == "" {
		errorCode = "channel_test_failed"
	}

	updated, err := updateChannelMetadata(channel.Id, []string{"response_time", "test_time", "other_info"}, func(current *Channel, info map[string]interface{}) error {
		now := common.GetTimestamp()
		info[ChannelLastTestErrorCodeKey] = errorCode
		info[ChannelLastTestErrorTimeKey] = now
		if strings.TrimSpace(errorMessage) == "" {
			delete(info, ChannelLastTestErrorMessageKey)
		} else {
			info[ChannelLastTestErrorMessageKey] = errorMessage
		}
		current.TestTime = now
		current.ResponseTime = 0
		return nil
	})
	if err != nil {
		common.SysLog(fmt.Sprintf("failed to update channel test failure: channel_id=%d, error=%v", channel.Id, err))
		return
	}
	channel.TestTime = updated.TestTime
	channel.ResponseTime = updated.ResponseTime
...
func (channel *Channel) Delete() error {
	return DB.Transaction(func(tx *gorm.DB) error {
		if err := DeleteChannelGroupRoutings(tx, []int{channel.Id}); err != nil {
			return err
		}
