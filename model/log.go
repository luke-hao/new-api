...
const LogExportLimit = 8000

var ErrLogExportLimitExceeded = errors.New("log export limit exceeded")
...
type Log struct {
	...
	UserId            int    `json:"user_id" gorm:"index;index:idx_user_id_id,priority:1;index:idx_logs_user_token_created,priority:1"`
	CreatedAt         int64  `json:"created_at" gorm:"bigint;index:idx_created_at_id,priority:1;index:idx_created_at_type;index:idx_logs_user_token_created,priority:3"`
	...
	TokenId           int    `json:"token_id" gorm:"default:0;index;index:idx_logs_user_token_created,priority:2"`
...
func formatUserLogs(logs []*Log, startIdx int) {
	for i := range logs {
		...
		content, topologyMasked := common.SanitizePublicErrorText(logs[i].Content)
		if topologyMasked {
		...
		if otherMap != nil {
			...
			delete(otherMap, "upstreamRequestId")
			delete(otherMap, "base_url")
			delete(otherMap, "baseURL")
			if topologyMasked {
				otherMap["error_type"] = string(types.ErrorTypeNewAPIError)
				otherMap["error_code"] = string(types.ErrorCodePoolChannelUnavailable)
			}
		}
...
func GetAllLogs(params LogQueryParams, startIdx int, num int) (logs []*Log, total int64, err error) {
...
func GetUserLogs(userId int, params LogQueryParams, startIdx int, num int) (logs []*Log, total int64, err error) {
	params.Username = ""
	params.Channel = 0
	tx, err := buildLogQuery(params)
	if err != nil {
...
func getLogsForExport(params LogQueryParams) ([]*Log, error) {
	tx, err := buildLogQuery(params)
	if err != nil {
		return nil, err
	}
	var logs []*Log
	if err = tx.Order("logs.created_at desc, logs.id desc").Limit(LogExportLimit + 1).Find(&logs).Error; err != nil {
		return nil, err
	}
	if len(logs) > LogExportLimit {
		return nil, ErrLogExportLimitExceeded
	}
	return logs, nil
}

func ExportAllLogs(params LogQueryParams) ([]*Log, error) {
	logs, err := getLogsForExport(params)
	if err != nil {
		return nil, err
	}
	if err = populateLogChannelNames(logs); err != nil {
...
func ExportUserLogs(userId int, params LogQueryParams) ([]*Log, error) {
	params.Username = ""
	params.Channel = 0
	tx, err := buildLogQuery(params)
	if err != nil {
	...
	var logs []*Log
	if err = tx.Where("logs.user_id = ?", userId).
		Order("logs.created_at desc, logs.id desc").
		Limit(LogExportLimit + 1).
