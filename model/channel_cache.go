...
func InitChannelCache() {
	...
	newGroup2model2channels := make(map[string]map[string][]ChannelCandidate)
	for _, ability := range abilities {
		...
		if _, ok := newGroup2model2channels[ability.Group]; !ok {
			newGroup2model2channels[ability.Group] = make(map[string][]ChannelCandidate)
		}
		priority := int64(0)
		...
		newGroup2model2channels[ability.Group][ability.Model] = append(
			...
			ChannelCandidate{ChannelId: ability.ChannelId, Priority: priority, Weight: int(ability.Weight)},
		)
	}

	// sort by priority
	for group, model2channels := range newGroup2model2channels {
		for model, candidates := range model2channels {
			sort.Slice(candidates, func(i, j int) bool {
				return candidates[i].Priority > candidates[j].Priority
			})
			newGroup2model2channels[group][model] = candidates
		}
	}
...
func GetRandomSatisfiedChannel(group string, model string, retry int) (*Channel, error) {
	...
	if len(candidates) == 1 {
		...
		return nil, fmt.Errorf("数据库一致性错误，渠道# %d 不存在，请联系管理员修复", candidates[0].ChannelId)
	}

	uniquePriorities := make(map[int]bool)
	for _, candidate := range candidates {
		if _, ok := channelsIDM[candidate.ChannelId]; ok {
			uniquePriorities[int(candidate.Priority)] = true
		} else {
	...
	var sumWeight = 0
	var targetCandidates []ChannelCandidate
	for _, candidate := range candidates {
		if _, ok := channelsIDM[candidate.ChannelId]; !ok {
			return nil, fmt.Errorf("数据库一致性错误，渠道# %d 不存在，请联系管理员修复", candidate.ChannelId)
		}
	...
	if sumWeight == 0 {
		...
		sumWeight = len(targetCandidates) * 100
		smoothingAdjustment = 100
	} else if sumWeight/len(targetCandidates) < 10 {
	...
	for _, candidate := range targetCandidates {
		...
		if randomWeight < 0 {
			return channelsIDM[candidate.ChannelId], nil
		}
	}
			...
			for model, candidates := range model2channels {
				for i, candidate := range candidates {
					if candidate.ChannelId == id {
						...
						group2model2channels[group][model] = append(candidates[:i], candidates[i+1:]...)
						break
					}
				}
			}
