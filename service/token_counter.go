...
func getImageToken(c *gin.Context, fileMeta *types.FileMeta, model string, stream bool) (int, error) {
	...
	if isPatchBased {
		...
		if rawPatches > 1536 {
			...
			imageTokens, err := common.SafeNonNegativeFloatToInt("image patch tokens", patchesW*patchesH)
			if err != nil {
				return 0, err
			}
			if imageTokens > 1536 {
			...
			return common.SafeNonNegativeFloatToInt("image tokens", math.Round(float64(imageTokens)*multiplier))
		}
		// below cap
	...
	fitW, err := common.SafeNonNegativeFloatToInt("image fit width", math.Round(float64(width)/fitScale))
	if err != nil {
		return 0, err
	}
	fitH, err := common.SafeNonNegativeFloatToInt("image fit height", math.Round(float64(height)/fitScale))
	if err != nil {
		return 0, err
	}
	...
	finalW, err := common.SafeNonNegativeFloatToInt("image final width", math.Round(float64(fitW)*shortScale))
	if err != nil {
		return 0, err
	}
	finalH, err := common.SafeNonNegativeFloatToInt("image final height", math.Round(float64(fitH)*shortScale))
	if err != nil {
		return 0, err
	}
...
func EstimateRequestToken(c *gin.Context, meta *types.TokenCountMeta, info *relaycommon.RelayInfo) (int, error) {
	...
	if info.RelayMode == constant2.RelayModeAudioTranscription || info.RelayMode == constant2.RelayModeAudioTranslation {
		...
		for _, fileHeader := range fileHeaders {
			...
			audioTokens, err := common.SafeNonNegativeFloatToInt("audio tokens", math.Round(math.Ceil(duration)/60.0*1000))
			if err != nil {
				return 0, err
			}
			totalAudioToken, err = common.SafeAddInt("audio tokens", totalAudioToken, audioTokens)
			if err != nil {
				return 0, err
			}
		}
