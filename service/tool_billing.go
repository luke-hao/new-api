...
func ComputeToolCallQuota(usage ToolCallUsage, groupRatio float64) ToolCallResult {
	...
	addItem := func(toolName string, count int) {
		...
		quota, err := common.SafeNonNegativeFloatToInt("tool call quota", math.Round(totalPrice*common.QuotaPerUnit*groupRatio))
		if err != nil {
			common.SysLog("error calculating tool call quota: " + err.Error())
			return
		}
		...
		totalQuota, err = common.SafeAddInt("tool call total quota", totalQuota, quota)
		if err != nil {
			common.SysLog("error calculating tool call total quota: " + err.Error())
			return
		}
	}
	...
	if usage.ImageGenerationCall {
		...
		quota, err := common.SafeNonNegativeFloatToInt("image generation tool quota", math.Round(price*common.QuotaPerUnit*groupRatio))
		if err != nil {
			common.SysLog("error calculating image generation tool quota: " + err.Error())
