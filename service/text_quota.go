...
func fallbackTextQuotaOnError(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, field string, err error) int {
	logger.LogError(ctx, fmt.Sprintf("error calculating %s: %s", field, err.Error()))
...
func safeRoundedTextQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, field string, value decimal.Decimal) int {
	quota, err := common.SafeDecimalToNonNegativeInt(field, value.Round(0))
	if err != nil {
		return fallbackTextQuotaOnError(ctx, relayInfo, field, err)
	}
	return quota
}

func composeTieredTextQuota(relayInfo *relaycommon.RelayInfo, summary textQuotaSummary, tieredQuota int, tieredResult *billingexpr.TieredResult) (int, error) {
	if tieredQuota < 0 {
		return 0, fmt.Errorf("tiered quota cannot be negative")
	}
	...
	if tieredResult != nil {
		if snap := relayInfo.TieredBillingSnapshot; snap != nil {
			return common.SafeDecimalToNonNegativeInt("tiered text quota", decimal.NewFromFloat(tieredResult.ActualQuotaBeforeGroup).
	...
	surcharge, err := common.SafeDecimalToNonNegativeInt("tool call surcharge quota", summary.ToolCallSurchargeQuota.Round(0))
	if err != nil {
		return 0, err
	}
	return common.SafeAddInt("tiered text quota", tieredQuota, surcharge)
}
...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if originUsage != nil {
		...
		if tieredOk {
			...
			composedQuota, err := composeTieredTextQuota(relayInfo, summary, tieredQuota, tieredRes)
			if err != nil {
				logger.LogError(ctx, "error composing tiered text quota: "+err.Error())
				composedQuota = tieredQuota
				if composedQuota < 0 {
					composedQuota = 0
				}
			}
			summary.Quota = composedQuota
		}
	}
