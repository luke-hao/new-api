...
func ClaudeErrorWrapper(err error, code string, statusCode int) *dto.ClaudeErrorWithStatusCode {
	...
	if sanitized, masked := common.SanitizePublicErrorText(text); masked {
		text = sanitized
	}
...
// IsUpstreamBalanceError recognizes billing failures returned by an upstream
...
func TaskErrorWrapper(err error, code string, statusCode int) *dto.TaskError {
	...
	if sanitized, masked := common.SanitizePublicErrorText(text); masked {
		text = sanitized
		code = common.PublicPoolChannelUnavailableCode
	}
	//避免暴露内部错误
...
func TaskErrorFromAPIError(apiErr *types.NewAPIError) *dto.TaskError {
	...
	if sanitized, masked := common.SanitizePublicErrorText(message); masked {
		message = sanitized
		code = common.PublicPoolChannelUnavailableCode
	}
