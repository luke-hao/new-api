...
const FRONTEND_VERSION_STORAGE_KEY = 'frontend_version'
const FRONTEND_VERSION_CHECK_INTERVAL_MS = 60_000
...
async function fetchFrontendVersion(): Promise<string | null> {
  ...
  const payload = (await response.json()) as {
    success?: boolean
    data?: { frontend_version?: unknown }
  }
  if (!payload.success || typeof payload.data?.frontend_version !== 'string') {
    return null
  }
  return payload.data.frontend_version
}

function useFrontendVersionRefresh() {
  useEffect(() => {
    let cancelled = false
    ...
    const checkVersion = async () => {
      ...
      const nextVersion = await fetchFrontendVersion().catch(() => null)
