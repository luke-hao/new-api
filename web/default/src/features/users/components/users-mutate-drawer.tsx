...
import { formatNumber, formatQuota, parseQuotaFromDollars } from '@/lib/format'
...
import { createUser, updateUser, getUser, getUserGroups } from '../api'
...
export function UsersMutateDrawer({
  ...
  const groupOptions = useMemo(() => {
    const options = [...(groupsData?.data ?? [])]
    const currentGroup = currentRow?.group?.trim()
    if (currentGroup && !options.some((group) => group.name === currentGroup)) {
      options.push({
        name: currentGroup,
        description: t('Unregistered legacy user group'),
        user_count: 1,
        billing_group: false,
      })
    }
    return options
  }, [currentRow?.group, groupsData?.data, t])
  ...
  const displayedUser = userDetails ?? currentRow
            ...
            >
              ...
              {isUpdate && (
                <SideDrawerSection>
                  ...
                  <FormField
                    control={form.control}
                    name='private_message_title'
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('Private Message Title')}</FormLabel>
                        <FormControl>
                          <Input
                            {...field}
                        ...
                        <FormDescription>
                          {t(
                            'Shown as a large popup when this user opens the site.'
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('Private Message Content')}</FormLabel>
                        <FormControl>
                          <Textarea
                            {...field}
                            placeholder={t(
                              'Leave empty and save to clear the private message.'
                            )}
                            rows={6}
                            maxLength={5000}
                          />
                        ...
                        <FormDescription>
                          {t('Supports Markdown. This is visible to the user.')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
              ...
              {/* Invite Information (Read-only) */}
              {isUpdate && (
                <SideDrawerSection>
                  <h3 className='text-sm font-medium'>{t('Invite Info')}</h3>
                  ...
                  <div className='grid gap-3 sm:grid-cols-3'>
                    <div>
                      ...
                      <Input
                        value={formatNumber(displayedUser?.invitee_count ?? 0)}
                        readOnly
                        className='mt-1'
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={formatNumber(
                          displayedUser?.invitee_total_recharge_money ?? 0
                        )}
                        readOnly
                        className='mt-1'
                      />
                    ...
                    <div>
                      ...
                      <Input
                        value={formatQuota(
                          displayedUser?.invitee_total_used_quota ?? 0
                        )}
                        readOnly
                        className='mt-1'
                      />
                  ...
                  <div className='flex flex-col gap-3'>
                    {BINDING_FIELDS.map(({ key, label }) => (
                      <div key={key}>
                        ...
                        <Input
                          value={
                            (displayedUser?.[key as keyof User] as string) ||
                            '-'
                          }
                          disabled
                          className='mt-1'
