...
import type {
  ...
  BroadcastPrivateMessagePayload,
  ApiResponse,
  AdminUserGroup,
} from './types'
...
/**
 * Broadcast a private popup message to all non-deleted users
 */
export async function broadcastPrivateMessage(
  payload: BroadcastPrivateMessagePayload
): Promise<ApiResponse<{ count: number }>> {
  const res = await api.post('/api/user/private_message/broadcast', payload)
  return res.data
}

/**
 * Clear the current global private popup message
 */
export async function clearBroadcastPrivateMessage(): Promise<
  ApiResponse<{ count: number }>
> {
  const res = await api.delete('/api/user/private_message/broadcast')
  return res.data
}
...
/**
 * Get registered user identity groups for administrator assignment.
 */
export async function getUserGroups(): Promise<ApiResponse<AdminUserGroup[]>> {
  const res = await api.get('/api/group/user')
  return res.data
}
