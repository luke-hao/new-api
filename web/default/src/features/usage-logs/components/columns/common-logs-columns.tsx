...
export function buildLogDetailSegments(
  log: UsageLog,
  ...
  columns.push(
...

    {
      ...
      cell: function DetailsCell({ row }) {
        ...
        const segments = buildLogDetailSegments(log, other, t)
        ...
        return (
          <>
            <button
              type='button'
              className='group flex max-w-[420px] min-w-[280px] items-start gap-2 text-left text-xs'
              onClick={() => setDialogOpen(true)}
              title={t('Click to view full details')}
            >
              <Eye className='text-muted-foreground mt-0.5 size-3.5 shrink-0' />
              {primary ? (
                <span
                  className={cn(
                    'max-h-20 min-w-0 flex-1 overflow-y-auto leading-snug break-words whitespace-normal group-hover:underline',
                    primary.muted
                ...
                <span className='text-muted-foreground max-h-20 min-w-0 flex-1 overflow-y-auto break-words whitespace-normal group-hover:underline'>
                  {log.content}
                </span>
              ) : (
                <span className='text-muted-foreground/40 min-w-0 flex-1'>
                  —
                </span>
              )}
              <span className='text-primary shrink-0 text-[11px] group-hover:underline'>
                {t('View')}
              </span>
            </button>
            <DetailsDialog
