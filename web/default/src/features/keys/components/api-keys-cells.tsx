...
export function ApiKeyGroupCell({
  ...
  const handleGroupChange = useCallback(
    async (nextGroup: string) => {
      ...
      try {
        const result = await batchUpdateApiKeyGroup({
          ...
          group: nextGroup,
          cross_group_retry:
            nextGroup === 'auto' && apiKey.cross_group_retry === true,
        })
    ...
    [apiKey.cross_group_retry, apiKey.id, group, isUpdating, t, triggerRefresh]
  )

  const hasCurrentGroup = options.some((option) => option.value === group)
  const availableOptions = hasCurrentGroup
    ? options
    : [{ value: group, label: group }, ...options]
  ...
  return (
    <div className='-ml-1.5 min-w-[148px]'>
      <ApiKeyGroupCombobox
        ...
        placeholder={group || t('Select a group')}
