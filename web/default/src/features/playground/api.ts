...
/**
 * Get image-capable groups and their group-specific models.
 */
export async function getImageCapabilities(): Promise<ImageGroupCapability[]> {
  const res = await api.get(API_ENDPOINTS.IMAGE_CAPABILITIES)
  const { data } = res
  ...
  return data.data as ImageGroupCapability[]
}
