...
/**
 * Load image playground config from localStorage
 */
export function loadImageConfig(): Partial<ImagePlaygroundConfig> {
  try {
...
/**
 * Save image playground config to localStorage
 */
export function saveImageConfig(config: Partial<ImagePlaygroundConfig>): void {
  try {
...
/**
 * Load image generation history from localStorage
 */
export function loadImageHistory(): ImageHistoryItem[] | null {
  try {
    const saved = localStorage.getItem(STORAGE_KEYS.IMAGE_HISTORY)
    if (saved) {
      const parsed: unknown = JSON.parse(saved)
      return Array.isArray(parsed) ? (parsed as ImageHistoryItem[]) : null
    }
  } catch (error) {
    // eslint-disable-next-line no-console
...
/**
 * Save image generation history to localStorage
 */
export function saveImageHistory(history: ImageHistoryItem[]): void {
  try {
    localStorage.setItem(STORAGE_KEYS.IMAGE_HISTORY, JSON.stringify(history))
  } catch (error) {
    // eslint-disable-next-line no-console
