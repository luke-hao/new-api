        ...
        <CollapsibleContent>
          <div className='space-y-2 border-t p-3'>
            {props.items.map((rule) => {
              const targetOptions = props.billingGroups.includes(
                rule.targetGroup
              )
                ? props.billingGroups
                : [rule.targetGroup, ...props.billingGroups].filter(Boolean)
              return (
                <div key={rule._id} className='flex items-center gap-2'>
                  ...
                  >
                    <SelectTrigger className='flex-1'>
                      <SelectValue placeholder={t('Select a billing group')} />
                    </SelectTrigger>
                    <SelectContent alignItemWithTrigger={false}>
                      <SelectGroup>
...
export function GroupSpecialUsableRulesEditor(
    ...
    (groupName: string) => {
      emitChange([
        ...rules,
        {
          ...
          userGroup: groupName,
          op: OP_APPEND,
          targetGroup: props.billingGroups[0] ?? '',
          description: '',
        },
      ])
    },
    [emitChange, props.billingGroups, rules]
  )

  const addNewGroup = useCallback(() => {
    const name = newGroupName.trim()
    if (!name) return
    if (!props.userGroups.includes(name)) return
    if (rules.some((rule) => rule.userGroup === name)) return
    emitChange([
      ...rules,
      {
        _id: uid(),
    ...
    setNewGroupName('')
  }, [emitChange, newGroupName, props.billingGroups, props.userGroups, rules])
  ...
  const availableUserGroups = props.userGroups.filter(
    (name) => !grouped.some((group) => group.name === name)
  )
      ...
      <CardContent>
        <div className='space-y-3'>
          ...
          <div className='flex items-center justify-center gap-2 pt-2'>
            <Select
              items={availableUserGroups.map((name) => ({
                value: name,
                label: name,
              }))}
              value={newGroupName}
            ...
            <Button
              ...
              size='sm'
              onClick={addNewGroup}
              disabled={!newGroupName || props.billingGroups.length === 0}
            >
