...
const createGroupSchema = (t: Translate) =>
  z.object({
    UserGroups: createJsonStringField(t),
...
export function RatioSettingsCard({
  ...
  const groupUpdateMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.success) {
        toast.success(t('Group settings updated successfully'))
        queryClient.invalidateQueries({ queryKey: ['system-options'] })
        queryClient.invalidateQueries({ queryKey: ['user-identity-groups'] })
      } else {
        toast.error(data.message || t('Failed to update group settings'))
      }
    },
    onError: (error: Error) => {
      toast.error(error.message || t('Failed to update group settings'))
    },
  })
  const {
    mutateAsync: updateGroupSettingsAsync,
    isPending: isGroupUpdatePending,
  } = groupUpdateMutation
  ...
  const groupNormalizedDefaults = useRef({
    UserGroups: normalizeJsonString(groupDefaults.UserGroups),
  ...
  const groupForm = useForm<GroupFormValues>({
    ...
    defaultValues: {
      ...groupDefaults,
      UserGroups: formatJsonForTextarea(groupDefaults.UserGroups),
  ...
  const saveGroupRatios = useCallback(
    async (values: GroupFormValues) => {
      const normalized = {
        UserGroups: normalizeJsonString(values.UserGroups),
      ...
      if (!changed) {
        toast.info(t('No group setting changes to save'))
        return
      }

      const result = await updateGroupSettingsAsync(normalized)
      if (result.success) {
        groupNormalizedDefaults.current = normalized
      }
    },
    [t, updateGroupSettingsAsync]
  )
