...
export const GroupRatioForm = memo(function GroupRatioForm({
  ...
  const userGroupNames = useMemo(
    () =>
      Object.keys(
        safeJsonParse<Record<string, string>>(userGroupsValue, {
          fallback: {},
          silent: true,
        })
      ).sort(),
  ...
  const billingGroupNames = useMemo(
    () =>
      Object.keys(
        safeJsonParse<Record<string, number>>(groupRatioValue, {
          fallback: {},
          silent: true,
        })
      ).sort(),
        ...
        {editMode === 'visual' ? (
          ...
          <SettingsForm onSubmit={form.handleSubmit(onSave)}>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('User identity groups')}</FormLabel>
                  <FormControl>
                    <Textarea rows={8} {...field} />
                  </FormControl>
                  <FormDescription>
                    {t(
                      'JSON map of user identity group identifiers to descriptions. These groups are assigned to users by administrators.'
                    )}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
          ...
          <section className='space-y-2'>
            ...
            <div className='text-muted-foreground space-y-2 text-sm leading-6'>
              ...
              <p>
                ...
                {': '}
                {t(
                  'When enabled, users can pick this billing group when creating tokens. This does not make it a user identity group.'
                )}
              </p>
            </div>
          </section>
...
internal     0.5     ${t('No')}                ${t('Granted by an administrator or a special rule')}`}
                </GuideCodeBlock>
                <p className='text-muted-foreground text-sm leading-6'>
                  {t(
                    'Non-selectable billing groups remain billing groups and can still be granted by administrators or special usable-group rules.'
                  )}
                </p>
              </AccordionContent>
            </AccordionItem>
