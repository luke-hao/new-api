...
export const GroupRatioVisualEditor = memo(function GroupRatioVisualEditor({
  ...
  const handleSimpleSave = (name: string, value: string) => {
    ...
    if (
      simpleDialogType === 'topupGroupRatio' &&
      !userGroupNames.includes(name)
    ) {
      return
    }
  ...
  const handleAutoGroupSave = () => {
    ...
    if (!billingGroupNames.includes(autoGroupInput.trim())) return
    if (autoGroupsList.includes(autoGroupInput.trim())) return
  ...
  const handleUserGroupSave = () => {
    ...
    if (!userGroupNames.includes(userGroupInput.trim())) return
    ...
    if (map[userGroupInput.trim()]) return
  ...
  const handleOverrideSave = (
    ...
    if (!billingGroupNames.includes(targetGroup)) return
  ...
  const topupGroupNames = new Set(topupRatioList.map((group) => group.name))
  const availableTopupGroups = userGroupNames.filter(
    (name) => !topupGroupNames.has(name)
  )
  const overrideUserGroups = new Set(
    groupGroupRatioList.map((group) => group.userGroup)
  )
  const availableOverrideUserGroups = userGroupNames.filter(
    (name) => !overrideUserGroups.has(name)
  )
  const availableAutoGroups = billingGroupNames.filter(
    (name) => !autoGroupsList.includes(name)
  )
  const currentOverrideTargets = new Set(
    groupGroupRatioList
      .find((group) => group.userGroup === groupOverrideUserGroup)
      ?.overrides.map((override) => override.targetGroup) ?? []
  )
  const availableOverrideTargets = billingGroupNames.filter(
    (name) =>
      !currentOverrideTargets.has(name) ||
      name === groupOverrideEditData?.targetGroup
  )
  ...
  return (
    <div className='space-y-4'>
      <UserGroupTable
        ...
        onChange={(value) => onChange('UserGroups', value)}
      />
        ...
        <CardContent>
          <div className='space-y-4'>
            <Button
              ...
              size='sm'
              disabled={availableTopupGroups.length === 0}
            >
        ...
        <CardContent>
          <div className='space-y-4'>
            <Button
              ...
              size='sm'
              disabled={availableOverrideUserGroups.length === 0}
            >
              <Plus className='mr-2 h-4 w-4' />
              {t('Add user group')}
            </Button>
        ...
        <CardContent>
          <div className='space-y-4'>
            <Button
              ...
              size='sm'
              disabled={availableAutoGroups.length === 0}
            >
              <Plus className='mr-2 h-4 w-4' />
              {t('Add group')}
            </Button>
      ...
      <SimpleGroupDialog
        ...
        groupOptions={simpleEditData ? userGroupNames : availableTopupGroups}
      />
        ...
        <div className='space-y-4 py-4'>
          <div className='space-y-2'>
            ...
            <GroupNameSelect
              ...
              options={availableOverrideUserGroups}
      ...
      <GroupOverrideDialog
        ...
        billingGroups={availableOverrideTargets}
      />
    </div>
  )
})
    ...
    <Card className={sectionCardClassName}>
      <CardHeader className={sectionHeaderClassName}>
        <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
          <div>
            <CardTitle>{t('Billing and token groups')}</CardTitle>
            <CardDescription>
              {t(
                'These groups control token routing and billing. User selectable only controls whether users can choose the billing group themselves.'
              )}
            </CardDescription>
          </div>
          ...
          <StaticDataTable
            ...
            columns={[
              ...
              },
              {
                ...
                className: 'w-36',
                cell: (row) =>
                  identityGroups.has(row.name.trim()) ? (
                    <StatusBadge variant='info' copyable={false}>
                      {t('Also a user group')}
                    </StatusBadge>
                  ) : (
                    <StatusBadge variant='neutral' copyable={false}>
                      {t('Billing group only')}
                    </StatusBadge>
                  ),
              },
              {
