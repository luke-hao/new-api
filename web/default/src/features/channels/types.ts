...
export interface ChannelSettings {
  ...
  claude_thinking_recovery_enabled?: boolean
...
export type UpstreamBillingProbeStatus =
  | 'ok'
  | 'partial'
  | 'unsupported'
  | 'failed'

export type UpstreamBillingKeyProbeStatus = Exclude<
  UpstreamBillingProbeStatus,
  'partial'
>

export interface UpstreamBillingData {
  object: 'sub2api.key_billing' | 'newapi.log_billing'
  schema_version: 1
  billing_scope: 'token'
...
export interface UpstreamBillingKeyResult {
  key_index: number
  key_mask: string
  key_fingerprint: string
  status: UpstreamBillingKeyProbeStatus
  http_status?: number
  error_code?: string
  attempted_at: number
  last_success_at?: number
  billing?: UpstreamBillingData
}
...
export interface UpstreamBillingProbeSnapshot {
  ...
  failed_count: number
  consistent: boolean
  effective_rate_min?: number
  effective_rate_max?: number
  key_results: UpstreamBillingKeyResult[]
  name_updated?: boolean
  channel_name?: string
}

export interface UpstreamBillingProbeResponse {
  success: boolean
  message?: string
  data?: UpstreamBillingProbeSnapshot
}
...
export interface ChannelGroupRoutingUpdateItem {
  channel_id: number
  priority?: number
  weight?: number
...
export interface ChannelGroupStabilityStatus {
  ...
  healthy_threshold_seconds: number
  ...
  last_result: string
  last_message: string
  last_primary_channel_id: number
  last_primary_latency_ms: number
  last_reordered_at: number
}
