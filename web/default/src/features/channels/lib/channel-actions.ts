...
async function runWithConcurrency<T>(
  values: number[],
  concurrency: number,
  operation: (value: number) => Promise<T>
): Promise<PromiseSettledResult<T>[]> {
  const results: PromiseSettledResult<T>[] = new Array(values.length)
  let nextIndex = 0
  const worker = async () => {
...
export async function handleUpdateChannelGroupRoutingField(
  channelIds: number[],
  group: string,
  ...
  if (!group || channelIds.length === 0) return
  const updates = channelIds.map((channelId) => ({
  ...
  try {
    const response = await updateChannelGroupRouting({ group, updates })
    if (response.success) {
      toast.success(
        inherit
          ? i18next.t(
              '{{field}} restored to the channel default for {{group}}',
              {
                field: i18next.t(
                  fieldName === 'priority' ? 'Priority' : 'Weight'
                ),
                group,
              }
            )
          : i18next.t('{{field}} updated to {{value}} for {{group}}', {
              field: i18next.t(
                fieldName === 'priority' ? 'Priority' : 'Weight'
              ),
              value,
...
export interface BatchUpstreamBillingProbeSummary {
  ok: number
  renamed: number
  inconsistent: number
  partial: number
...
function addUpstreamBillingProbeResult(
  summary: BatchUpstreamBillingProbeSummary,
  ...
  if (snapshot.name_updated) summary.renamed++
  if (snapshot.status === 'ok') {
    if (snapshot.consistent) summary.ok++
    else summary.inconsistent++
    return
  }
  summary[snapshot.status]++
}

/** Probe any number of concrete channels, with at most three requests in flight. */
export async function handleBatchProbeUpstreamBilling(
  ids: number[],
  queryClient?: QueryClient,
  onSuccess?: (summary: BatchUpstreamBillingProbeSummary) => void
): Promise<void> {
  ...
  const summary: BatchUpstreamBillingProbeSummary = {
    ok: 0,
    renamed: 0,
    inconsistent: 0,
    partial: 0,
  ...
  results.forEach((result) => {
    ...
    addUpstreamBillingProbeResult(
      summary,
      result.value.success ? result.value.data : undefined
    )
  })
  await queryClient?.invalidateQueries({ queryKey: channelsQueryKeys.lists() })
  const message = i18next.t(
    'Upstream multiplier sync completed: {{ok}} normal, {{renamed}} names updated, {{inconsistent}} inconsistent, {{partial}} partial, {{unsupported}} unsupported, {{failed}} failed.',
    {
      ok: summary.ok,
