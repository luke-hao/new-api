...
import type {
  ...
  ChannelSettings,
  UpstreamBillingData,
  UpstreamBillingKeyProbeStatus,
  UpstreamBillingKeyResult,
  UpstreamBillingProbeSnapshot,
  UpstreamBillingProbeStatus,
} from '../types'
...
export type ChannelLastTestError = {
  code: string
  message?: string
}

const LAST_TEST_ERROR_CODE_KEY = 'last_test_error_code'
const LAST_TEST_ERROR_MESSAGE_KEY = 'last_test_error_message'
const LAST_TEST_ERROR_TIME_KEY = 'last_test_error_time'
...
const UPSTREAM_BILLING_PROBE_KEY = 'upstream_billing_probe'
const UPSTREAM_BILLING_STATUSES = new Set<UpstreamBillingProbeStatus>([
  'ok',
  'partial',
  'unsupported',
  'failed',
])
const UPSTREAM_BILLING_KEY_STATUSES = new Set<UpstreamBillingKeyProbeStatus>([
  'ok',
  'unsupported',
  'failed',
])
...
function parseUpstreamBillingData(value: unknown): UpstreamBillingData | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null
  const data = value as Record<string, unknown>
  if (
    (data.object !== 'sub2api.key_billing' &&
      data.object !== 'newapi.log_billing') ||
    data.schema_version !== 1 ||
    data.billing_scope !== 'token' ||
    !isFiniteNonNegativeNumber(data.group_rate_multiplier) ||
    !isFiniteNonNegativeNumber(data.resolved_rate_multiplier) ||
    typeof data.peak_rate_enabled !== 'boolean' ||
    !isFiniteNonNegativeNumber(data.effective_rate_multiplier) ||
    typeof data.observed_at !== 'string'
  ) {
    return null
  }
  return data as unknown as UpstreamBillingData
}

function parseUpstreamBillingKeyResult(
  value: unknown
): UpstreamBillingKeyResult | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null
  const result = value as Record<string, unknown>
  if (
    typeof result.key_index !== 'number' ||
    typeof result.key_mask !== 'string' ||
    typeof result.key_fingerprint !== 'string' ||
    !UPSTREAM_BILLING_KEY_STATUSES.has(
      result.status as UpstreamBillingKeyProbeStatus
    ) ||
    !isFiniteNonNegativeNumber(result.attempted_at)
  ) {
    return null
  }
  const billing =
    result.billing === undefined
      ? undefined
      : (parseUpstreamBillingData(result.billing) ?? undefined)
  if (result.billing !== undefined && !billing) return null
  return {
    ...(result as unknown as UpstreamBillingKeyResult),
    billing,
  }
}

export function parseUpstreamBillingProbeSnapshot(
  otherInfo?: string | null
): UpstreamBillingProbeSnapshot | null {
  const raw = parseChannelOtherInfo(otherInfo)[UPSTREAM_BILLING_PROBE_KEY]
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null
  const snapshot = raw as Record<string, unknown>
  if (
    !UPSTREAM_BILLING_STATUSES.has(
      snapshot.status as UpstreamBillingProbeStatus
    ) ||
    !isFiniteNonNegativeNumber(snapshot.attempted_at) ||
    !isFiniteNonNegativeNumber(snapshot.total_keys) ||
    !isFiniteNonNegativeNumber(snapshot.success_count) ||
    !isFiniteNonNegativeNumber(snapshot.unsupported_count) ||
    !isFiniteNonNegativeNumber(snapshot.failed_count) ||
    typeof snapshot.consistent !== 'boolean' ||
    (snapshot.name_updated !== undefined &&
      typeof snapshot.name_updated !== 'boolean') ||
    (snapshot.channel_name !== undefined &&
      typeof snapshot.channel_name !== 'string') ||
    !Array.isArray(snapshot.key_results)
  ) {
    return null
  }
  const keyResults = snapshot.key_results.map(parseUpstreamBillingKeyResult)
  if (keyResults.some((result) => result === null)) return null
  return {
    ...(snapshot as unknown as UpstreamBillingProbeSnapshot),
    key_results: keyResults as UpstreamBillingKeyResult[],
  }
}

export function updateUpstreamBillingProbeInfo(
  otherInfo: string | null | undefined,
  snapshot: UpstreamBillingProbeSnapshot
): string {
  const parsed = parseChannelOtherInfo(otherInfo)
  parsed[UPSTREAM_BILLING_PROBE_KEY] = snapshot
  return JSON.stringify(parsed)
}

export function formatUpstreamBillingRate(value: number): string {
  return new Intl.NumberFormat(undefined, {
    maximumFractionDigits: 6,
  }).format(value)
}

export function getChannelLastTestError(
  otherInfo?: string | null
): ChannelLastTestError | null {
  const parsed = parseChannelOtherInfo(otherInfo)
  const code = parsed[LAST_TEST_ERROR_CODE_KEY]

  if (typeof code !== 'string' || code.trim() === '') {
    return null
  }

  const message = parsed[LAST_TEST_ERROR_MESSAGE_KEY]
  return {
    code: code.trim(),
    message:
...
export function updateChannelLastTestErrorInfo({
  otherInfo,
  errorCode,
  errorMessage,
  errorTime,
  ...
  if (normalizedCode) {
    parsed[LAST_TEST_ERROR_CODE_KEY] = normalizedCode
    if (errorMessage?.trim()) {
      parsed[LAST_TEST_ERROR_MESSAGE_KEY] = errorMessage.trim()
    } else {
      delete parsed[LAST_TEST_ERROR_MESSAGE_KEY]
    }
    parsed[LAST_TEST_ERROR_TIME_KEY] =
      errorTime ?? Math.floor(Date.now() / 1000)
  } else {
    delete parsed[LAST_TEST_ERROR_CODE_KEY]
    delete parsed[LAST_TEST_ERROR_MESSAGE_KEY]
    delete parsed[LAST_TEST_ERROR_TIME_KEY]
  }
...
export function aggregateChannelsByTag(
  ...
  for (const channel of channels) {
    ...
    const channelPriority = useEffectiveRouting
      ? (channel.effective_priority ?? channel.priority)
      : channel.priority
    const channelWeight = useEffectiveRouting
      ? (channel.effective_weight ?? channel.weight)
      : channel.weight
    ...
    if (tagRow.priority === -1) {
      tagRow.priority = channelPriority
    } else if (tagRow.priority !== channelPriority) {
      tagRow.priority = null
    }

    // Aggregate weight (same value or null if different)
    if (tagRow.weight === -1) {
      tagRow.weight = channelWeight
    } else if (tagRow.weight !== channelWeight) {
      tagRow.weight = null
    }

    if (useEffectiveRouting) {
      tagRow.priority_overridden =
        Boolean(tagRow.priority_overridden) ||
        Boolean(channel.priority_overridden)
      tagRow.weight_overridden =
        Boolean(tagRow.weight_overridden) || Boolean(channel.weight_overridden)
    }
