...
import { DollarSign, Gauge, Power, PowerOff, Tag, Trash2 } from 'lucide-react'
...
import {
  ...
  handleBatchDisable,
  handleBatchEnable,
  handleBatchProbeUpstreamBilling,
  handleBatchSetTag,
...
export function DataTableBulkActions({
  ...
  const [isProbingBilling, setIsProbingBilling] = useState(false)
  ...
  const handleProbeBilling = async () => {
    if (isProbingBilling) return
    setIsProbingBilling(true)
    try {
      await handleBatchProbeUpstreamBilling(selectedIds, queryClient)
    } finally {
      setIsProbingBilling(false)
    }
  }
  ...
  const handleUpdateAll = async () => {
    ...
    try {
      await handleUpdateAllBalances(queryClient)
    } finally {
    ...
    <>
      ...
      >
        <Tooltip>
          <TooltipTrigger
            render={
              <Button
                variant='outline'
                size='sm'
                onClick={() => void handleProbeBilling()}
                disabled={isProbingBilling}
                className='h-8 px-2'
                aria-label={t(
                  'Sync upstream multiplier and names for selected channels'
                )}
                title={t(
                  'Sync upstream multiplier and names for selected channels'
                )}
          ...
          >
            <Gauge
              className={isProbingBilling ? 'size-4 animate-pulse' : 'size-4'}
            />
            <span>{t('Sync multiplier and names')}</span>
          </TooltipTrigger>
          <TooltipContent>
            <p>
              {t('Sync upstream multiplier and names for selected channels')}
            </p>
          </TooltipContent>
        </Tooltip>
          ...
          <TooltipTrigger
            render={
              <Button
                ...
                className='h-8 px-2'
                aria-label={t('Update All Balances')}
                title={t('Update All Balances')}
              />
            }
          >
            <DollarSign
              className={
                isUpdatingBalances ? 'size-4 animate-pulse' : 'size-4'
              }
            />
            <span>{t('Update All Balances')}</span>
          </TooltipTrigger>
          <TooltipContent>
            <p>{t('Update All Balances')}</p>
          </TooltipContent>
        </Tooltip>
