...
export function ChannelsTable() {
  ...
  const channels = useMemo(() => {
    ...
    if (enableTagMode && rawChannels.length > 0) {
      return aggregateChannelsByTag(rawChannels, Boolean(selectedGroup))
    }

    return rawChannels
  }, [data, enableTagMode, selectedGroup])
  ...
  useEffect(() => {
    // Selection belongs to one filtered result set and should not leak into another.
  ...
  const handleSelectAllMatching = useCallback(
    async (checked: boolean) => {
      ...
      if (enableTagMode || totalCount === 0 || isSelectingAll) return
      ...
      try {
        ...
        setRowSelection(Object.fromEntries(ids.map((id) => [String(id), true])))
      } catch {
        clearSelection()
      } finally {
  ...
  const columns = useChannelsColumns(selectedGroup, {
    checked: totalCount > 0 && selectedIds.length === totalCount,
    indeterminate: selectedIds.length > 0 && selectedIds.length < totalCount,
    disabled: enableTagMode || totalCount === 0 || isSelectingAll,
    onCheckedChange: (checked) => void handleSelectAllMatching(checked),
  })
  ...
  const { table } = useDataTable({
    ...
    getRowId: (row: Channel) => String(row.id),
