...
import type {
  ...
  ChannelGroupRoutingUpdateParams,
...
type GetAllChannelIdsParams = GetChannelsParams &
  Pick<SearchChannelsParams, 'keyword' | 'model'>

/** Fetch every concrete channel ID matching the active list filters. */
export async function getAllChannelIds(
  params: GetAllChannelIdsParams,
  ...
  while (ids.size < total) {
    const requestParams = { ...params, p: page, page_size: pageSize }
    const response = shouldSearch
      ? await searchChannels(requestParams)
      : await getChannels(requestParams)
    ...
    response.data.items.forEach((channel) => ids.add(channel.id))

    if (
      response.data.items.length === 0 ||
      response.data.items.length < pageSize ||
      ids.size === previousSize
    ) {
      break
    }
    page++
...
export async function updateChannelGroupRouting(
  data: ChannelGroupRoutingUpdateParams
): Promise<{
  success: boolean
  message?: string
  data?: { updated: number }
...
export async function updateChannelGroupStability(
  data: ChannelGroupStabilityConfig
): Promise<{
  success: boolean
  message?: string
  data?: ChannelGroupStabilityStatus
...
/**
 * Synchronize the upstream Sub2API or NewAPI billing multiplier for a channel.
 */
export async function probeChannelUpstreamBilling(
  id: number
): Promise<UpstreamBillingProbeResponse> {
  const res = await api.post(
    `/api/channel/upstream-billing-probe/${id}`,
    undefined,
    channelActionConfig()
  )
  return res.data
