...
import type {
  ...
  WalletRechargeReminderRequest,
  WalletRechargeReminderResponse,
} from './types'
...
/**
 * Update wallet recharge reminder switch (admin only)
 */
export async function updateWalletRechargeReminder(
  request: WalletRechargeReminderRequest
): Promise<WalletRechargeReminderResponse> {
  const res = await api.put('/api/user/topup/reminder', request)
  return res.data
}
...
export async function getAffiliateRebates(
  page: number,
  pageSize: number
): Promise<AffiliateRebateHistoryResponse> {
