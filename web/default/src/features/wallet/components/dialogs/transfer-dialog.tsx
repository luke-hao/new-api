...
export function TransferDialog({
  ...
  const minimumAmount = quotaUnitsToDollars(quotaPerUnit)
  ...
  const canTransfer =
    ...
    requestedQuota >= quotaPerUnit &&
    requestedQuota <= availableQuota
  ...
  const handleConfirm = async () => {
    ...
    const success = await onConfirm(requestedQuota)
    if (success) {
      onOpenChange(false)
    }
  }
      ...
      <div className='space-y-4 py-3 sm:space-y-6 sm:py-4'>
        ...
        <div className='space-y-3'>
          ...
          <p className='text-muted-foreground text-xs'>
            {t('Minimum:')} {formatQuota(quotaPerUnit)}
          </p>
        </div>
      </div>
    </Dialog>
