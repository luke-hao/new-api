...
import type {
  ...
  ExportRedemptionsResponse,
  DeleteRedemptionsBatchParams,
  RedemptionFormData,
} from './types'
...
export async function getRedemptions(
  ...
  const res = await api.get('/api/redemption/', {
    params: buildRedemptionQueryParams({ ...params, p, page_size }),
  })
  return res.data
}
...
export async function searchRedemptions(
  ...
  const res = await api.get('/api/redemption/search', {
    params: buildRedemptionQueryParams({ ...params, keyword, p, page_size }),
  })
  return res.data
}

// Export redemption codes matching current filters
export async function exportRedemptions(
  params: GetRedemptionsParams = {}
): Promise<ExportRedemptionsResponse> {
  const res = await api.get('/api/redemption/export', {
    params: buildRedemptionQueryParams(params),
  })
  return res.data
}
...
// Delete selected or filtered redemption codes
export async function deleteRedemptionsBatch(
  data: DeleteRedemptionsBatchParams
): Promise<ApiResponse<number>> {
  const res = await api.delete('/api/redemption/batch', { data })
  return res.data
}
