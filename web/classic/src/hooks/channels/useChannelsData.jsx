...
const updateChannelTestOtherInfo = (otherInfo, errorCode, errorMessage) => {
  let parsed = {};
  if (otherInfo) {
    try {
      const maybeParsed = JSON.parse(otherInfo);
      if (
        maybeParsed &&
  ...
  if (normalizedCode) {
    parsed.last_test_error_code = normalizedCode;
    parsed.last_test_error_time = Math.floor(Date.now() / 1000);
    if (typeof errorMessage === 'string' && errorMessage.trim() !== '') {
      parsed.last_test_error_message = errorMessage.trim();
    } else {
      delete parsed.last_test_error_message;
    }
  } else {
    delete parsed.last_test_error_code;
    delete parsed.last_test_error_message;
    delete parsed.last_test_error_time;
  }
...
export const useChannelsData = () => {
  ...
  const testChannel = async (
    ...
    try {
      ...
      const latestErrorCode = success ? null : error_code || 'channel_test_failed';
      ...
      if (success) {
        ...
        updateChannelProperty(record.id, (channel) => {
          ...
          channel.other_info = updateChannelTestOtherInfo(channel.other_info);
        });
        ...
        updateChannelProperty(record.id, (channel) => {
          ...
          channel.test_time = Date.now() / 1000;
          channel.other_info = updateChannelTestOtherInfo(
            channel.other_info,
            latestErrorCode,
            message,
          );
      ...
      // 处理网络错误
      const testKey = `${record.id}-${model}`;
      const errorMessage = error.message || t('缃戠粶閿欒');
      setModelTestResults((prev) => ({
        ...prev,
        [testKey]: {
          success: false,
