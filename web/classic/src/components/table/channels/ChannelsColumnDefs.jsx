...
const getChannelLastTestError = (record) => {
  if (!record || record.children !== undefined || !record.other_info) {
    return null;
  }
  try {
    const otherInfo = JSON.parse(record.other_info);
    const code = otherInfo?.last_test_error_code;
    if (typeof code !== 'string' || code.trim() === '') {
      return null;
    }
    const message = otherInfo?.last_test_error_message;
    return {
      code: code.trim(),
      message:
...
const renderResponseTime = (responseTime, t, record) => {
  const lastTestError = getChannelLastTestError(record);
  if (lastTestError) {
    const errorTag = (
      <Tag color='red' shape='circle'>
        {lastTestError.code}
    ...
    return lastTestError.message ? (
      <Tooltip content={lastTestError.message}>{errorTag}</Tooltip>
    ) : (
      errorTag
    );
  }
...
export const getChannelsColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <div>{renderResponseTime(text, t, record)}</div>
      ),
    },
    {
