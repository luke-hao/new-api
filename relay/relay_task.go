...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if !common.StringsContains(constant.TaskPricePatches, modelName) {
		for _, ra := range info.PriceData.OtherRatios {
			if ra != 1.0 {
				info.PriceData.Quota, err = common.SafeMultiplyQuotaByRatio(info.PriceData.Quota, ra, "task quota")
				if err != nil {
					return nil, service.TaskErrorWrapperLocal(err, "invalid_task_quota", http.StatusBadRequest)
				}
			}
		}
	}
	...
	if adjustedRatios := adaptor.AdjustBillingOnSubmit(info, taskData); len(adjustedRatios) > 0 {
		...
		finalQuota, err = recalcQuotaFromRatios(info, adjustedRatios)
		if err != nil {
			return nil, service.TaskErrorWrapperLocal(err, "invalid_task_quota", http.StatusBadRequest)
		}
...
func recalcQuotaFromRatios(info *relaycommon.RelayInfo, ratios map[string]float64) (int, error) {
	...
	for _, ra := range info.PriceData.OtherRatios {
		if ra != 1.0 && ra > 0 {
			var err error
			baseQuota, err = common.SafeDivideQuotaByRatio(baseQuota, ra, "task base quota")
			if err != nil {
				return 0, err
			}
		}
	...
	for _, ra := range ratios {
		if ra != 1.0 {
			var err error
			result, err = common.SafeMultiplyQuotaByRatio(result, ra, "task adjusted quota")
			if err != nil {
				return 0, err
			}
		}
