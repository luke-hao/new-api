...
func TestClaudeHelperRecoversInvalidThinkingSignatureOnSameChannel(t *testing.T) {
	...
	common.SetContextKey(c, constant.ContextKeyChannelSetting, dto.ChannelSettings{ClaudeThinkingRecoveryEnabled: true})
	...
	preflight, err := service.SanitizeKnownInvalidClaudeThinking(requestBody, 777)
	require.NoError(t, err)
