...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (types.PriceData, error) {
	...
	if !usePrice {
		...
		preConsumedQuota, err = common.SafeMultiplyQuotaByRatio(preConsumedTokens, ratio, "pre-consumed quota")
		if err != nil {
			return types.PriceData{}, err
		}
	} else {
		if !imageSizePriceOverride && meta.ImagePriceRatio != 0 {
			modelPrice = modelPrice * meta.ImagePriceRatio
		}
		preConsumedQuota, err = common.SafeNonNegativeFloatToInt("pre-consumed quota", modelPrice*common.QuotaPerUnit*groupRatioInfo.GroupRatio)
		if err != nil {
			return types.PriceData{}, err
		}
	}
	...
	priceData := types.PriceData{
		...
		UsePrice:               usePrice,
		ImageSizePriceOverride: imageSizePriceOverride,
		ImageSizePriceTier:     imageSizePriceTier,
...
func ModelPriceHelperPerCall(c *gin.Context, info *relaycommon.RelayInfo) (types.PriceData, error) {
	...
	if usePrice {
		var err error
		quota, err = common.SafeNonNegativeFloatToInt("task quota", modelPrice*common.QuotaPerUnit*groupRatioInfo.GroupRatio)
		if err != nil {
			return types.PriceData{}, err
		}
		if !operation_setting.GetQuotaSetting().EnableFreeModelPreConsume {
		...
		// 按量计费：以模型倍率的一半作为预扣额度
		var err error
		quota, err = common.SafeNonNegativeFloatToInt("task quota", modelRatio/2*common.QuotaPerUnit*groupRatioInfo.GroupRatio)
		if err != nil {
			return types.PriceData{}, err
		}
		modelPrice = -1
...
func modelPriceHelperTiered(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta, groupRatioInfo types.GroupRatioInfo) (types.PriceData, error) {
	...
	preConsumedQuota, err := common.SafeNonNegativeFloatToInt("tiered pre-consumed quota", math.Round(quotaBeforeGroup*groupRatioInfo.GroupRatio))
	if err != nil {
		return types.PriceData{}, err
	}
