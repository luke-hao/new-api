...
func GetAndValidateResponsesRequest(c *gin.Context) (*dto.OpenAIResponsesRequest, error) {
	...
	if err := common.ValidateOptionalUintLimit("max_output_tokens", request.MaxOutputTokens, common.MaxRequestTokens); err != nil {
		return nil, err
	}
	if err := common.ValidateOptionalUintLimit("max_tool_calls", request.MaxToolCalls, common.MaxToolCalls); err != nil {
		return nil, err
	}
	return request, nil
}
...
func GetAndValidOpenAIImageRequest(c *gin.Context, relayMode int) (*dto.ImageRequest, error) {
	...
	case relayconstant.RelayModeImagesEdits:
		if strings.Contains(c.Request.Header.Get("Content-Type"), "multipart/form-data") {
			...
			if rawN := strings.TrimSpace(formData.Get("n")); rawN != "" {
				n, err := strconv.ParseUint(rawN, 10, 0)
				if err != nil {
					return nil, fmt.Errorf("n is invalid")
				}
			...
			if err := validateImageRequestQuantities(imageRequest); err != nil {
				return nil, err
			}
...
func validateImageRequestQuantities(imageRequest *dto.ImageRequest) error {
	if imageRequest == nil {
		return nil
	}
	return common.ValidateOptionalUintLimit("n", imageRequest.N, common.MaxImageGenerationCount)
}
...
func GetAndValidateClaudeRequest(c *gin.Context) (textRequest *dto.ClaudeRequest, err error) {
	...
	if err := common.ValidateOptionalUintLimit("max_tokens", textRequest.MaxTokens, common.MaxRequestTokens); err != nil {
		return nil, err
	}
	if err := common.ValidateOptionalUintLimit("max_tokens_to_sample", textRequest.MaxTokensToSample, common.MaxRequestTokens); err != nil {
		return nil, err
	}
...
func GetAndValidateTextRequest(c *gin.Context, relayMode int) (*dto.GeneralOpenAIRequest, error) {
	...
	if err := common.ValidateOptionalUintLimit("max_completion_tokens", textRequest.MaxCompletionTokens, common.MaxRequestTokens); err != nil {
		return nil, err
	}
	if textRequest.N != nil {
		if err := common.ValidateIntRange("n", *textRequest.N, 1, common.MaxCompletionChoices); err != nil {
			return nil, err
		}
	}
	if textRequest.Model == "" {
...
func GetAndValidateGeminiRequest(c *gin.Context) (*dto.GeminiChatRequest, error) {
	...
	if err := common.ValidateOptionalUintLimit("generationConfig.maxOutputTokens", request.GenerationConfig.MaxOutputTokens, common.MaxRequestTokens); err != nil {
		return nil, err
	}
	if request.GenerationConfig.CandidateCount != nil && *request.GenerationConfig.CandidateCount != 0 {
		if err := common.ValidateIntRange("generationConfig.candidateCount", *request.GenerationConfig.CandidateCount, 1, common.MaxCompletionChoices); err != nil {
			return nil, err
		}
	}
