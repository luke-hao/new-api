...
func (a *Adaptor) ConvertOpenAIRequest(c *gin.Context, info *relaycommon.RelayInfo, request *dto.GeneralOpenAIRequest) (any, error) {
	...
	if a.RequestMode == RequestModeGemini && strings.HasPrefix(info.UpstreamModelName, "imagen") {
		...
		if request.N != nil && *request.N > 0 {
			if err := common.ValidateIntRange("image n", *request.N, 1, int(common.MaxImageGenerationCount)); err != nil {
				return nil, err
			}
		...
		if len(request.ExtraBody) > 0 {
			...
			if err := json.Unmarshal(request.ExtraBody, &extra); err == nil {
				if n, ok := extra["n"].(float64); ok && n > 0 {
					imageN, err := common.SafeNonNegativeFloatToInt("image n", n)
					if err != nil {
						return nil, err
					}
					if err := common.ValidateIntRange("image n", imageN, 1, int(common.MaxImageGenerationCount)); err != nil {
						return nil, err
					}
					imgReq.N = lo.ToPtr(uint(imageN))
				}
