...
func RequestOpenAI2ClaudeMessage(c *gin.Context, textRequest dto.GeneralOpenAIRequest) (*dto.ClaudeRequest, error) {
	...
	if baseModel, effortLevel, ok := reasoning.TrimEffortSuffix(textRequest.Model); ok && effortLevel != "" &&
		...
		if strings.HasPrefix(trimmedModel, "claude-opus-4-7") ||
			...
			budgetTokens, err := common.SafeNonNegativeFloatToInt("thinking budget tokens", float64(*claudeRequest.MaxTokens)*model_setting.GetClaudeSettings().ThinkingAdapterBudgetTokensPercentage)
			if err != nil {
				return nil, err
			}
			claudeRequest.Thinking = &dto.Thinking{
...
func shouldSkipClaudeMessageDeltaUsagePatch(info *relaycommon.RelayInfo) bool {
	return shouldUseClaudeResponsePassthrough(info)
}

func shouldUseClaudeResponsePassthrough(info *relaycommon.RelayInfo) bool {
