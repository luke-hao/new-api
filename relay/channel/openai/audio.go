...
func OpenaiTTSHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) *dto.Usage {
	...
	if info.IsStream {
		...
		if durationErr != nil {
			...
			estimatedTokens, err := common.SafeNonNegativeFloatToInt("estimated audio tokens", math.Ceil(sizeInKB)) // 粗略估算每KB约等于1 token
			if err != nil {
				logger.LogError(c, "failed to estimate audio tokens: "+err.Error())
			} else {
			...
			completionTokens, err := common.SafeNonNegativeFloatToInt("audio completion tokens", math.Round(math.Ceil(duration)/60.0*1000))
			if err != nil {
				logger.LogError(c, "failed to calculate audio completion tokens: "+err.Error())
			} else {
				usage.CompletionTokens = completionTokens
				usage.CompletionTokenDetails.AudioTokens = completionTokens
			}
		}
		totalTokens, err := common.SafeAddInt("audio total tokens", usage.PromptTokens, usage.CompletionTokens)
		if err != nil {
			logger.LogError(c, "failed to calculate audio total tokens: "+err.Error())
		} else {
