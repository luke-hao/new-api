...
- 稳定调度以精确请求模型名和分组为键，自动枚举渠道模型。首次全测；以后主通道完整测试耗时不超过健康阈值则保持整个排名，否则仅全测当前模型。
- 15% 防抖只保护原第一名，其他成功渠道按本轮耗时升序、渠道 ID 打破同速排序；失败渠道为 0，全部失败保留原排名。
- `channel_model_stability_policies` 保存模型暂停、逐项参数覆盖和运行状态；`channel_model_routings` 保存独立优先级、固定状态及检测结果。`abilities` 是可重建的实际路由投影，模型覆盖优先于分组及渠道默认，但分组固定优先级约束全部模型。
- 分组开关是总开关；停止自动检测保留排名。全局探测并发 4，排队不计入耗时。分组及模型版本、渠道配置快照共同拒绝在途旧结果，观察记录和排名在同一事务提交。
- `group-stability`、`group-stability/run`、`group-routing` 接口支持可选 `model`；列表的 `routing_model` 是精确调度模型，与模糊搜索 `model` 独立。权重仍是分组设置。
- 默认主题渠道页提供模型选择、模型总览与模型级编辑。模型视图暂停标签聚合，单模型操作不会写入其他模型。Classic 保留现有管理界面，实际请求同样采用新路由。
- 启动时恢复模型优先级投影。回滚旧镜像前停止应用并把 abilities 重建为分组默认；保留新增表，重新上线自动恢复模型覆盖。不要回滚账务数据库。

## 分组固定优先级（2026-09-08）

- 分组路由表的 `priority_locked` 默认关闭，仅当前渠道与分组组合生效；锁定会把当前有效优先级保存为分组覆盖值。
- 固定渠道退出稳定性定时检测、立即全测及其重排；正常业务路由、状态控制、单独测试不受影响。
- `PUT /api/channel/group-routing` 支持可选 `priority_locked` 和 `mode: manual | rerank`（缺省 manual）；rerank 由后端重新检查锁定状态并返回 `skipped_locked`。
- 人工修改数值保留锁定，解除固定保留数值且不立即发起检测；恢复默认优先级前先解锁，权重恢复独立。
...
## 个人 Auto 分组（2026-09-26）

- 见 [API 密钥个人 Auto 顺序](../token-auto-groups.md)：每把密钥的 auto_groups 有序列表、文字及生图候选识别、重试及会话约束、实际分组计费。
- 列表省略保留旧值；空列表旧记录沿用管理员 Auto。回滚旧镜像前记录并停用个人 Auto 密钥，重新上线后恢复状态。
