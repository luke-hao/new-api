...
const (
	native4KImageGroup             = "生图分组-image2-4k(原生)"
	native4KImageGroupSizeErrorMsg = "生图分组-image2-4k(原生)仅支持严格大于 2K 的图片尺寸（不包含 2K）：请使用 4K，或最长边大于 2560 的自定义尺寸"
	upstreamBalancePublicMessage   = "渠道暂时无号，已封禁"
)
...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	defer func() {
		if newAPIError != nil {
			cleanError := common.StripRequestIDAnnotations(newAPIError.Error())
			logger.LogError(c, fmt.Sprintf("relay error: %s", common.LocalLogPreview(cleanError)))
	...
	defer func() {
		...
		if !maskUpstreamBalanceError {
			if publicErr, masked := service.MaskPublicUpstreamTopologyError(newAPIError); masked {
				newAPIError = publicErr
			}
			return
		}
		...
		newAPIError = types.NewErrorWithStatusCode(errors.New(upstreamBalancePublicMessage), newAPIError.GetErrorCode(), statusCode,
...
func fastTokenCountMetaForPricing(request dto.Request) *types.TokenCountMeta {
	...
	case *dto.GeneralOpenAIRequest:
		...
		if maxCompletionTokens > maxTokens {
			meta.MaxTokens = common.BoundedUintToInt(maxCompletionTokens, common.MaxRequestTokens)
		} else {
			meta.MaxTokens = common.BoundedUintToInt(maxTokens, common.MaxRequestTokens)
		}
	case *dto.OpenAIResponsesRequest:
		meta.MaxTokens = common.BoundedUintToInt(lo.FromPtrOr(r.MaxOutputTokens, uint(0)), common.MaxRequestTokens)
	case *dto.ClaudeRequest:
		meta.MaxTokens = common.BoundedUintToInt(lo.FromPtr(r.MaxTokens), common.MaxRequestTokens)
...
func validateNative4KImageRequest(info *relaycommon.RelayInfo, request dto.Request) error {
	if info == nil || info.UsingGroup != native4KImageGroup || info.OriginModelName != "gpt-image-2" {
		return nil
	}

	imageRequest, ok := request.(*dto.ImageRequest)
	if !ok {
		return errors.New(native4KImageGroupSizeErrorMsg)
	}
	if !dto.IsGPTImage2SizeStrictlyAbove2K(imageRequest.Size) {
		return errors.New(native4KImageGroupSizeErrorMsg)
	}
	return nil
}
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	cleanError := common.StripRequestIDAnnotations(err.Error())
	logger.LogError(c, fmt.Sprintf("channel error (channel #%d, status code: %d): %s", channelError.ChannelId, err.StatusCode, common.LocalLogPreview(cleanError)))
	// 不要使用context获取渠道信息，异步处理时可能会出现渠道信息不一致的情况
	// do not use context to get channel info, there may be inconsistent channel info when processing asynchronously
	if service.ShouldDisableChannel(err) && channelError.AutoBan {
		gopool.Go(func() {
			service.DisableChannel(channelError, err.ErrorWithStatusCode(), service.ChannelDisableReason(err))
		})
	}
	...
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) {
		...
		logContent := common.StripRequestIDAnnotations(err.MaskSensitiveErrorWithStatusCode())
