...
func testChannelWithContext(parent context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	if parent == nil {
...
func channelTestErrorCode(result testResult) string {
	...
	return "channel_test_failed"
}

func channelTestErrorMessage(result testResult) string {
...
func TestChannel(c *gin.Context) {
	...
	tik := time.Now()
	result := testChannelWithContext(c.Request.Context(), channel, testUserID, testModel, endpointType, isStream)
	if result.localErr != nil {
		channel.UpdateTestFailure(channelTestErrorCode(result), result.localErr.Error())
		resp := gin.H{
			"success": false,
	...
	consumedTime := float64(milliseconds) / 1000.0
	if result.newAPIError != nil {
		channel.UpdateTestFailure(channelTestErrorCode(result), result.newAPIError.Error())
...
func testAllChannels(notify bool) error {
	...
	gopool.Go(func() {
		...
		for _, channel := range channels {
			if channel.Status == common.ChannelStatusManuallyDisabled || (channel.Status == common.ChannelStatusAutoDisabled && operation_setting.GetChannelRecoveryPolicy().Enabled) {
				continue
			}
			...
			if result.localErr != nil || newAPIError != nil {
				...
				errorMessage := channelTestErrorMessage(result)
				if newAPIError != nil {
				...
				channel.UpdateTestFailure(errorCode, errorMessage)
			} else {
				channel.UpdateResponseTime(milliseconds)
			}
