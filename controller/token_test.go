...
func configureTokenGroupBatchTestSettings(t *testing.T) {
	t.Helper()

	savedUsableGroups := setting.UserUsableGroups2JSONString()
	savedGroupRatios := ratio_setting.GroupRatio2JSONString()
	t.Cleanup(func() {
		if err := setting.UpdateUserUsableGroupsByJSONString(savedUsableGroups); err != nil {
			t.Errorf("failed to restore user usable groups: %v", err)
		}
		if err := ratio_setting.UpdateGroupRatioByJSONString(savedGroupRatios); err != nil {
			t.Errorf("failed to restore group ratios: %v", err)
		}
	})
	...
	if err := setting.UpdateUserUsableGroupsByJSONString(`{
		...
		t.Fatalf("failed to configure user usable groups: %v", err)
	}
	if err := ratio_setting.UpdateGroupRatioByJSONString(`{
		"default":1,
		"premium":2
	}`); err != nil {
		t.Fatalf("failed to configure group ratios: %v", err)
	}
}

func setTokenGroupBatchContext(ctx *gin.Context) {
	common.SetContextKey(ctx, constant.ContextKeyUserGroup, "default")
}

func TestUpdateTokenGroupBatchUpdatesOwnedTokensOnly(t *testing.T) {
	configureTokenGroupBatchTestSettings(t)
	db := setupTokenControllerTestDB(t)

	first := seedToken(t, db, 1, "first-token", "first1234token5678")
	second := seedToken(t, db, 1, "second-token", "second123token5678")
	foreign := seedToken(t, db, 2, "foreign-token", "foreign123token5678")

	first.CrossGroupRetry = true
	first.RemainQuota = 321
	first.ModelLimitsEnabled = true
	first.ModelLimits = "model-a,model-b"
	if err := db.Save(first).Error; err != nil {
		t.Fatalf("failed to customize first token: %v", err)
	}

	body := map[string]any{
		"ids":               []int{first.Id, second.Id, first.Id, foreign.Id},
		"group":             "premium",
		"cross_group_retry": true,
	}
	ctx, recorder := newAuthenticatedContext(t, http.MethodPut, "/api/token/batch/group", body, 1)
	setTokenGroupBatchContext(ctx)
	...
	var count int
	if err := common.Unmarshal(response.Data, &count); err != nil {
		t.Fatalf("failed to decode update count: %v", err)
	}
	if count != 2 {
		t.Fatalf("expected 2 updated tokens, got %d", count)
	}

	var updatedFirst, updatedSecond, unchangedForeign model.Token
	if err := db.First(&updatedFirst, first.Id).Error; err != nil {
		t.Fatalf("failed to reload first token: %v", err)
	}
	if err := db.First(&updatedSecond, second.Id).Error; err != nil {
		t.Fatalf("failed to reload second token: %v", err)
	}
	if err := db.First(&unchangedForeign, foreign.Id).Error; err != nil {
		t.Fatalf("failed to reload foreign token: %v", err)
	}

	for _, token := range []model.Token{updatedFirst, updatedSecond} {
		if token.Group != "premium" {
			t.Errorf("expected premium group, got %q", token.Group)
		}
		if token.CrossGroupRetry {
			t.Errorf("expected cross-group retry to be disabled for non-auto group")
		}
	}
	if updatedFirst.Name != "first-token" || updatedFirst.RemainQuota != 321 || !updatedFirst.ModelLimitsEnabled || updatedFirst.ModelLimits != "model-a,model-b" {
		t.Fatalf("batch group update changed unrelated token fields: %+v", updatedFirst)
	}
	if unchangedForeign.Group != "default" {
		t.Fatalf("foreign token group changed to %q", unchangedForeign.Group)
	}
}

func TestUpdateTokenGroupBatchSupportsAutoRetry(t *testing.T) {
	configureTokenGroupBatchTestSettings(t)
	db := setupTokenControllerTestDB(t)
	token := seedToken(t, db, 1, "auto-token", "auto12345token5678")
	...
	if updated.Group != "auto" || !updated.CrossGroupRetry {
		t.Fatalf("expected auto group with cross-group retry enabled, got group=%q retry=%v", updated.Group, updated.CrossGroupRetry)
	}
}

func TestUpdateTokenGroupBatchRejectsInvalidRequests(t *testing.T) {
	configureTokenGroupBatchTestSettings(t)
	setupTokenControllerTestDB(t)

	tooManyIDs := make([]int, maxTokenBatchSize+1)
	...
	tests := []struct {
		...
		{name: "empty ids", body: map[string]any{"ids": []int{}, "group": "premium"}},
		{name: "too many ids", body: map[string]any{"ids": tooManyIDs, "group": "premium"}},
		{name: "unavailable group", body: map[string]any{"ids": []int{1}, "group": "missing"}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctx, recorder := newAuthenticatedContext(t, http.MethodPut, "/api/token/batch/group", tt.body, 1)
			setTokenGroupBatchContext(ctx)
			UpdateTokenGroupBatch(ctx)
