...
func GetUser(c *gin.Context) {
	...
	if err := model.FillUserInviteeStats(user); err != nil {
		common.ApiError(c, err)
		return
	}
	user.PrivateMessage = user.GetSetting().PrivateMessage
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
		"data":    user,
...
func GetSelf(c *gin.Context) {
	...
	privateMessage := model.GetEffectivePrivateMessage(userSetting)
...
func UpdateUser(c *gin.Context) {
	...
	if updatedUser.Group != originUser.Group && !setting.ContainsUserGroup(updatedUser.Group) {
		common.ApiErrorMsg(c, fmt.Sprintf("用户分组未登记：%s", updatedUser.Group))
		return
	}
	myRole := c.GetInt("role")
	...
	if updatedUser.PrivateMessage != nil {
		privateMessage, err := normalizePrivateMessage(originUser, updatedUser.PrivateMessage)
		if err != nil {
			common.ApiErrorMsg(c, err.Error())
			return
		}
		currentSetting := originUser.GetSetting()
		currentSetting.PrivateMessage = privateMessage
...
func normalizePrivateMessage(originUser *model.User, message *dto.PrivateMessage) (*dto.PrivateMessage, error) {
	if message == nil {
		return nil, nil
	}
	...
	return buildPrivateMessage(title, content, originUser.GetSetting().PrivateMessage)
}

func buildPrivateMessage(title string, content string, currentMessage *dto.PrivateMessage) (*dto.PrivateMessage, error) {
	if content == "" {
		return nil, nil
	}
	if len([]rune(title)) > 80 {
		return nil, fmt.Errorf("私信标题不能超过80个字符")
	}
	if len([]rune(content)) > 5000 {
		return nil, fmt.Errorf("私信内容不能超过5000个字符")
	}

	messageId := ""
	createdAt := int64(0)
	if currentMessage != nil && currentMessage.Title == title && currentMessage.Content == content {
		messageId = currentMessage.Id
		createdAt = currentMessage.CreatedAt
	}
	if messageId == "" {
		messageId = fmt.Sprintf("%d-%s", common.GetTimestamp(), common.GetRandomString(8))
	}
	if createdAt <= 0 {
		createdAt = common.GetTimestamp()
	}

	return &dto.PrivateMessage{
		Id:        messageId,
		Title:     title,
		Content:   content,
		CreatedAt: createdAt,
...
func BroadcastPrivateMessage(c *gin.Context) {
	...
	privateMessage, err := buildPrivateMessage(strings.TrimSpace(req.Title), strings.TrimSpace(req.Content), nil)
	if err != nil {
		common.ApiErrorMsg(c, err.Error())
		return
	}
	if privateMessage == nil {
		common.ApiErrorMsg(c, "私信内容不能为空")
		return
	}

	count, err := model.BroadcastPrivateMessage(privateMessage)
	if err != nil {
		common.ApiError(c, err)
		return
	}

	recordManageAuditFor(c, 0, "user.private_message_broadcast", map[string]interface{}{
		"count": count,
...
func ClearBroadcastPrivateMessage(c *gin.Context) {
	count, err := model.ClearGlobalPrivateMessage()
	if err != nil {
		common.ApiError(c, err)
		return
	}

	recordManageAuditFor(c, 0, "user.private_message_broadcast_clear", map[string]interface{}{
		"count": count,
	})
	common.ApiSuccess(c, gin.H{"count": count})
}
...
func UpdateUserSetting(c *gin.Context) {
	...
	settings := dto.UserSetting{
		...
		PrivateMessage:                   existingSettings.PrivateMessage,
