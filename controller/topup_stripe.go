...
func StripeWebhook(c *gin.Context) {
	...
	if processingErr != nil {
		logger.LogError(ctx, fmt.Sprintf("Stripe webhook 本地处理失败 event_type=%s client_ip=%s error=%q", string(event.Type), callerIp, processingErr.Error()))
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) error {
	if len(referenceId) == 0 {
		...
		return errors.New("missing client reference id")
	}
...
func getStripeMinTopup() int64 {
	...
	if operation_setting.GetQuotaDisplayType() == operation_setting.QuotaDisplayTypeTokens {
		quotaMinTopup, err := common.SafeDecimalToNonNegativeInt("minimum stripe topup quota", decimal.NewFromInt(int64(minTopup)).Mul(decimal.NewFromFloat(common.QuotaPerUnit)))
		if err != nil {
			common.SysLog("error calculating minimum stripe topup quota: " + err.Error())
