...
func GetTopUpInfo(c *gin.Context) {
	...
	data := gin.H{
		...
		"wallet_recharge_reminder_enabled": operation_setting.GetPaymentSetting().WalletRechargeReminderEnabled,
		"affiliate_topup_rebate_percent":   model.GetAffiliateTopupRebatePercent().InexactFloat64(),
...
func GetAffiliateRebates(c *gin.Context) {
	...
	items, total, err := model.GetUserAffiliateRebates(c.GetInt("id"), pageInfo)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	if total > int64(common.MaxSafeInt()) {
		common.ApiErrorMsg(c, "返利记录数量超出范围")
		return
	}
...
type WalletRechargeReminderRequest struct {
	Enabled *bool `json:"enabled"`
}

func UpdateWalletRechargeReminder(c *gin.Context) {
	var req WalletRechargeReminderRequest
	...
	err := model.UpdateOption(
		"payment_setting.wallet_recharge_reminder_enabled",
...
func getMinTopup() int64 {
	...
	if operation_setting.GetQuotaDisplayType() == operation_setting.QuotaDisplayTypeTokens {
		...
		dQuotaPerUnit := decimal.NewFromFloat(common.QuotaPerUnit)
		quotaMinTopup, err := common.SafeDecimalToNonNegativeInt("minimum topup quota", dMinTopup.Mul(dQuotaPerUnit))
		if err != nil {
			common.SysLog("error calculating minimum topup quota: " + err.Error())
			return int64(common.MaxSafeInt())
		}
		minTopup = quotaMinTopup
	}
