...
func SearchChannels(c *gin.Context) {
	...
	if enableTagMode {
		...
		for _, tag := range tags {
			if tag != nil && *tag != "" {
				var tagChannels []*model.Channel
				err := sortOptions.ApplyForGroup(buildChannelListQuery(group, -1, -1).Where("tag = ?", *tag), group).
					Omit("key").
					Find(&tagChannels).Error
				if err != nil {
					c.JSON(http.StatusOK, gin.H{
	...
	if err := model.PopulateEffectiveChannelRoutings(pagedData, group); err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "获取分组路由配置失败，请稍后重试",
		})
		return
	}
...
type ChannelGroupRoutingUpdateItem struct {
...
func UpdateChannelGroupRouting(c *gin.Context) {
	request := ChannelGroupRoutingUpdateRequest{}
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "参数错误"})
		return
	}
	patches := make([]model.ChannelGroupRoutingPatch, 0, len(request.Updates))
	for _, update := range request.Updates {
		patches = append(patches, model.ChannelGroupRoutingPatch{
...
func CopyChannel(c *gin.Context) {
	...
	if err := model.CopyChannelGroupRoutings(origin.Id, clone.Id); err != nil {
		_ = clone.Delete()
		common.SysError("failed to copy channel group routings: " + err.Error())
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "复制渠道分组路由失败，请稍后重试"})
		return
	}
	if err := clone.UpdateAbilities(nil); err != nil {
		_ = clone.Delete()
		common.SysError("failed to rebuild copied channel abilities: " + err.Error())
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "复制渠道能力配置失败，请稍后重试"})
		return
	}
	model.InitChannelCache()
